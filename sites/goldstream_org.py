...
def showSeries():# Danke Lynx187
    ...
    for name in aResult[1]:
        oGuiElement = cGuiElement(util.unescape(name.decode('utf-8')).encode('utf-8'), SITE_IDENTIFIER, 'showSeasons')
...
def showSeasons():# Danke Lynx187
    ...
    for link, name in aResult[1]:
        oGuiElement = cGuiElement(util.unescape(name.decode('utf-8')).encode('utf-8'), SITE_IDENTIFIER, 'showEpisodes')
...
def showEpisodes(entryUrl = False, sGui = False):
    oGui = sGui if sGui else cGui()
    params = ParameterHandler()
    if not entryUrl: entryUrl = params.getValue('sUrl')
    sHtmlContent = cRequestHandler(entryUrl).request()
    pattern = '<h2[^>]*class="entry-title"[^>]*><a[^>]*href="([^"]+)[^>]*rel="bookmark".*?Episode (\d+[^"]+).*?' # link / title
    ...
    total = len(aResult[1])
    for sEntryUrl, sName, sDescription in sorted(aResult[1]) :
        oGuiElement = cGuiElement(cUtil().unescape("Folge - " + sName.decode('utf-8')).encode('utf-8'), SITE_IDENTIFIER, 'showHosters')
